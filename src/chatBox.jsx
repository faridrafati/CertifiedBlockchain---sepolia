...
class ChatBox extends resetProvider {
    ...
    editContactListHandler = async (index) => {
        ...
        await Contract.methods.editMyContactList(contacts[selectedContactIndex].address,!contacts[selectedContactIndex].listed).send({from: (account), gas: '1000000'},(error,result) => {
            if(!error){
                TxId=result;
                this.notify('info','Editing Contact List is in Progress');
                }else{
                console.log(error);
                this.notify('error','Editing Contact List is Failed: '+error.message);
                }
            
            });
        this.notify('success','Editing Contact List is Done: '+TxId);
        contacts[selectedContactIndex].listed = !contacts[selectedContactIndex].listed;
        this.setState({contacts});
        await this.extraInitContract();
    }
    ...
    clearContactList = async () => {
        let TxId='';
        let {Contract,account} = this.state;
        await Contract.methods.clearMyContactList().send({from: (account), gas: '1000000'},(error,result) => {
            if(!error){
                TxId=result;
   ...
   render() {
        let {contacts,messages,account, checkRegister,owner,selectedContactIndex,searchValue,inputValue,showListedContact} = this.state;
